$(function () {
  ...
  $(".turnplatw_btn").click(function () {
    if (last_prize.frequency > 0) {
      ...
      $(".showS>.bg>img").attr(
        "src",
        "https://files.alexhchu.com/2020/09/14/8a7d4913709f9.png"
      );
      $(".showS>.bg>p").text("你没有次数了就不要点了！");
      $(".showS>.bg").css(
        "background",
        'url("https://7.dusays.com/2020/09/17/61a4b7efe8be2.png")no-repeat'
      );
  ...
  function publish_prize(a, list) {
    ...
    if (last_prize.ption == -1) {
      ...
      var chulia =
        parseInt(last_prize.ption * Math.pow(10, 3) + 0.5, 10) /
          Math.pow(10, 3) -
        Math.floor(last_prize.ption);
      var enff = a + chulia;
  ...
  function showS(list) {
    ...
    var jsonsrc = JSON.stringify(list);
    // 发送抽奖结果
    var userPhone = $(".inputOne").val();
    var settings = {
